...
export const validateColections = (colection, colectionsValids = []) => {
  const include = colectionsValids.includes(colection);
  if (!include) {
    throw new Error(
      `Colection is not valid, colections valids ${colectionsValids}`
    );
  }
  return true;
};
